...
public class TreeGenerator extends WorldGenAbstractTree {
    ...
    public TreeGenerator(IBlockState log, IBlockState leave, boolean useExtraRandomHeightIn)
    {
        super(false);
        LOG = log.withProperty(BlockLogs.LOG_AXIS, BlockLog.EnumAxis.X);
        LEAF = leave;
                ...
                if (isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
                    ...
                    for (int j2 = 0; j2 < i; ++j2)
                    {
                        ...
                        if (state2.getBlock().isAir(state2, worldIn, upN) || state2.getBlock().isLeaves(state2, worldIn, upN))
                        {
                            ...
                            worldIn.setBlockState(position.up(j2), worldIn.getBlockState(position.up(j2)).withProperty(BlockLogs.LOG_AXIS, BlockLog.EnumAxis.Y));
                        }
                    }
