...
public class StrengthSpell extends SpellBase {
    @Override
    public boolean onRightClick() {
        float amount = (float) (10F * (1.0F + 0.04F * SpellHelper.getMending(getOwner())));
        SkillUtils.spawnParticleCircle(getOwner().world, getOwner().getPositionVector(), EnumParticleTypes.VILLAGER_ANGRY, 0.6);
        SkillUtils.spawnParticleCircle(getOwner().world, getOwner().getPositionVector(), EnumParticleTypes.DRAGON_BREATH, 0.6);
        SkillUtils.playSoundFromServer(getOwner().world, getOwner().getPositionVector(), SoundEvents.ENTITY_ZOMBIE_VILLAGER_CURE, SoundCategory.PLAYERS, 1, 1);
        if(getOwner() instanceof EntityLivingBase){
            ((EntityLivingBase) getOwner()).addPotionEffect(new PotionEffect(PotionsHandler.STRENGTH, 20 * 30, (int)amount));
            return true;
        }
        return false;
    }
